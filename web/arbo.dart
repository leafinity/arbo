...
List<List<int>> chessboard = new List(8);
DivElement black_score, white_score;
DivElement clicked, cb, memo;
int times = 0;

void main() {
	cb = query(".chessboard");
	memo = query("#memo");
	writeMemo("Black first");
	black_score = query(".score_b");
	white_score = query(".score_w");
	createBoard();
	initScore();
	for(int i = 0; i < 8; i++){
		chessboard[i] = new List(8); 
	}
	chessboard[3][3] = chessboard[4][4] = BLACK;
	chessboard[3][4] = chessboard[4][3] = WHITE;
...
void setChess(int x, int y, int color){
	...
	clicked.nodes.add(new Element.html('<div class="${color == BLACK? "b": "w"}c"></div>'));
	clicked.classes.remove('can_set');
	for(int i = 0; i < 8; i++){
		if(x == 0 && (i == 0 || i == 1 || i == 7))
			continue;
		if(y == 0 && (i == 5 || i == 6 || i == 7))
